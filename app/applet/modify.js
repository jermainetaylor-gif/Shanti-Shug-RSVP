...
<body class="font-serif text-[#8c5a4d] antialiased">
  ...
  <div class="relative z-10 max-w-lg mx-auto w-full min-h-screen py-10 px-4 flex flex-col items-center">
    ...
    <div class="w-full glass-card shadow-glow rounded-t-[100px] rounded-b-3xl p-6 relative overflow-hidden flex flex-col items-center">
      ...
      <!-- PHONE ENTRY ENVELOPE -->
      <div id="view-landing" class="w-full envelope-container shadow-sm mt-8 relative z-20">
        <div class="text-center paper-slide-up">
          <p class="font-sans text-sm tracking-widest font-bold text-dustypink mb-6">UNLOCK INVITATION</p>
          <p class="font-serif italic text-lg mb-6">Enter your phone number to slide open your envelope.</p>
          <input type="tel" id="phone" placeholder="(555) 123-4567" maxlength="14" oninput="formatPhone(this)" class="w-full text-center p-4 border border-dustypink/30 rounded-full font-serif text-xl focus:outline-none focus:border-dustypink transition mb-2 bg-[#fcf6f7]">
      ...
      <div id="view-rsvp" class="hidden w-full paper-slide-up bg-white rounded-3xl p-6 md:p-8 mt-6 shadow-sm border border-dustypink/20">
        ...
        <!-- Step 1: Attending? -->
        <div id="step-1">
          <label class="font-sans text-xs font-bold tracking-widest uppercase text-dustypink mb-4 hidden">Will you be attending?</label>
        ...
        <div id="step-2" class="hidden mt-8 paper-slide-up space-y-6">
          <div id="group-questions" class="hidden space-y-4 border-t border-dustypink/20 pt-6">
          ...
          <div id="plus-one-group" class="hidden space-y-4 border-t border-dustypink/20 pt-6">
          ...
          <div id="plus-one-name-group" class="hidden space-y-2">
            ...
            <input type="text" id="plus-one-name" placeholder="Name" class="w-full p-4 rounded-xl border border-dustypink/30 font-serif text-lg bg-[#fcf6f7] outline-none focus:border-dustypink">
            <div id="plus-one-error" class="hidden text-xs font-sans tracking-wide text-red-400">Please provide a name.</div>
          </div>

          <button onclick="submitRSVP()" class="shimmer-btn w-full py-4 rounded-full font-sans font-bold text-sm tracking-[0.2em] uppercase mt-8">Finish RSVP</button>
        </div>
      ...
      <div id="view-denied" class="hidden w-full text-center mt-12 mb-12">
        ...
        <p class="font-serif italic text-xl mb-8">We couldn't find your number. Please try again or reach out to the host!</p>
      ...
      <!-- SUCCESS -->
      <div id="view-success" class="hidden w-full text-center mt-12 mb-12">
  ...
  <script>
    ...
    function checkPhone() {
      ...
      const capturedPhone = document.getElementById('phone').value;

      showView('loading');
      google.script.run
        .withSuccessHandler((response) => handlePhoneFound(response, capturedPhone))
        .withFailureHandler(handleError)
        .checkPhone(phone);
    }

    function handlePhoneFound(response, originalPhoneInput) {
      if (response.status === 'found') {
        currentGuest = {
          phone: originalPhoneInput,
          name: response.name,
          maxGuests: parseInt(response.maxGuests, 10) || 1,
          plusOneAllowed: response.plusOneAllowed === true || response.plusOneAllowed === 'true' || response.plusOneAllowed === 'yes'
        };
    ...
    function toggleGroupOptions() {
      ...
      } else if (currentGuest.maxGuests === 1 && currentGuest.plusOneAllowed) {
    ...
    function submitRSVP() {
      ...
      if (attending === 'Yes') {
         if (currentGuest.maxGuests > 1) {
           ...
           if (!allAttending) return document.getElementById('all-attending-error').classList.remove('hidden');
           
           if (allAttending === 'Yes') {
              numAttending = currentGuest.maxGuests;
           } else {
              const specNum = parseInt(document.getElementById('number-attending').value, 10);
              if (!specNum || isNaN(specNum)) return document.getElementById('number-attending-error').classList.remove('hidden');
              numAttending = specNum;
           }
           ...
           if (!bringingPlusOne) return document.getElementById('bringing-plus-one-error').classList.remove('hidden');
           
           if (bringingPlusOne === 'Yes') {
              numAttending = 2;
              const poName = document.getElementById('plus-one-name').value;
              if (!poName || poName.trim() === '') return document.getElementById('plus-one-error').classList.remove('hidden');
    ...
    function fireConfetti() {
      if(typeof confetti !== 'undefined') {
        const count = 200;
        const defaults = { origin: { y: 0.7 }, colors: ['#D69A9A', '#ffffff', '#ebd1d5'] };
...
fs.writeFileSync('/app/applet/src/App.tsx', finalContent);
